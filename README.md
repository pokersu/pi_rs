...
| [`pi-agent`](crates/pi-agent/AGENT.md) | `packages/agent` | 89/90 文件（除 B 类） | Agent 核心 + 全部 harness（工具/session/compaction/skills） |
...
- **pi-telemetry**、**pi-agent**：文件级 1:1 复刻。方法级差异仅为 Rust 语言固有限制（类型体操、`AsyncIterable` → `Vec`、`getValue<T>` 泛型方法因 `dyn` trait 无法复刻）。产品逻辑无缺失；仅 B 类（Node 平台细节、conformance 测试、legacy-v3 迁移）未搬，见 `todos.md`。
